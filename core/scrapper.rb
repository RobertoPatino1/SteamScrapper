  ...
  def extract_games
    ...
    loop do
      ...
      games.each do |game|
        ...
        puts "#{title}, #{release_date}, #{price}, #{platform_info}, #{reviews}"
...
def extract_discount_info(game)
  discount_percent = game.css('.search_discount.responsive_secondrow').text.strip
  discount_price = game.css('.search_price .discount_final_price').text.strip
